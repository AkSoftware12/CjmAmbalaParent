...
import '../AlumniChatScreen/alumni_admin_chat.dart';

class AlumniInboxScreen extends StatefulWidget {
  final int? messageSendPermissionsApp;

  const AlumniInboxScreen({
    super.key,
    required this.messageSendPermissionsApp,
  });

  @override
  State<AlumniInboxScreen> createState() => _AlumniInboxScreenState();
}

class _AlumniInboxScreenState extends State<AlumniInboxScreen> {
  ...
  Future<void> fetchAssignmentsData() async {
    ...
    try {
      ...
      if (selectedFilter == "all") {
        url = Uri.parse(
          '${ApiRoutes.getAdminAlumniInbox}?page=$currentPage',
        );
      } else {
        url = Uri.parse(
          '${ApiRoutes.getAdminAlumniInbox}?page=$currentPage&filter=$selectedFilter',
        );
      }
  ...
  void filterMessages(String query) {
    ...
    setState(() {
      filteredMessages = messsage.where((message) {
        final alumniName =
        (message['alumni_name'] ?? '').toString().toLowerCase();
        final lastMessage =
        (message['latest_message']?['message'] ?? '')
            .toString()
            .toLowerCase();

        return alumniName.contains(searchQuery) ||
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color:
                currentPage < totalPages ? Colors.white24 : Colors.white10,
                shape: BoxShape.circle,
              ),
              child: Icon(
                Icons.chevron_right,
  ...
  Widget _buildMessageCard(dynamic alumni) {
    DateTime? dateTime;
    String formatted = '';

    final latestMessage = alumni['latest_message'];

    try {
      if (latestMessage?['created_at'] != null) {
        dateTime = DateTime.parse(latestMessage['created_at'].toString());
    ...
    final unreadCount =
        int.tryParse((alumni['unread_count'] ?? '0').toString()) ?? 0;

    final String lastMessageText;
    if (latestMessage?['message'] != null &&
        latestMessage['message'].toString().trim().isNotEmpty) {
      lastMessageText = latestMessage['message'].toString();
    } else if (latestMessage?['attachment'] != null &&
        latestMessage['attachment'].toString().isNotEmpty) {
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => AlumniAdminChatScreen(
              alumniId: alumni['alumni_id'].toString(),
            ),
          ),
        ).then((value) {
          fetchAssignmentsData();
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    ClipRRect(
                      borderRadius: BorderRadius.circular(8.sp),
                      child: alumni['alumni_photo'] != null &&
                          alumni['alumni_photo'].toString().isNotEmpty
                          ? Image.network(
                        alumni['alumni_photo'].toString(),
                        height: 40.sp,
                        width: 40.sp,
                        fit: BoxFit.cover,
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              if (latestMessage?['attachment'] != null &&
            ...
            child: isLoading
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final alumni = filteredMessages[index];
                  return _buildMessageCard(alumni);
                },
              ),
            ),
          ),
