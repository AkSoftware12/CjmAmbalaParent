...
class _NewTeacherMessageScreenState extends State<ComposeMessageScreen>
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      receivers.addAll(
        selectedStudents.where((e) => e != null).map((e) => e.toString()),
      );
      ...
      // ✅ BACKGROUND JOB
        ...
        print('Api Send Msg Job key ${jobKey}');

        if (jobKey.isEmpty) {
          _showErrorSnackBar("Api Send Msg Something went wrong");
        ...
        _showErrorSnackBar('Api Send Msg Failed: ${response.statusCode}');
        setState(() => isSending = false);
      }
    } catch (e) {
      _showErrorSnackBar('Api Send Msg Error: $e');
      setState(() => isSending = false);
    } finally {
  ...
  void _toggleSelection(int id) {
    setState(() {
      if (_tabController.index == 0) {
        ...
        selectAllTeachers = selectedTeachers.length == filteredMessages.length;
      } else {
        ...
        selectAllStudents = selectedStudents.length == filteredMessages2.length;
      ...
      if (_tabController.index == 0) {
        if (selectAllTeachers) {
          ...
          selectedTeachers.addAll(
            filteredMessages.map((u) => "user_${u['id']}"),
          );
        ...
        if (selectAllStudents) {
          ...
          selectAllStudents =
              selectedStudents.length ==
                  filteredMessages2.length; // ✅ length se derive hoga
          debugPrint(
                    ...
                    children: [
                      ...
                      Checkbox(
                        value: _tabController.index == 0
                            ? selectAllTeachers
                            : selectedStudents.length ==
                            filteredMessages2.length &&
                            filteredMessages2.isNotEmpty,
                        onChanged: (value) => _toggleSelectAll(),
                        activeColor: AppColors.primary,
                      ),
                      Text(
